...
def display_settings(settings):
    ...
    st.subheader("Datenbankeinstellungen")
    edited_db_settings = {
        k: st.text_input(k, str(v), type="password") if k == "password" else st.text_input(k, str(v))
        for k, v in db_settings.items()
    } # {k: to_int_if_possible(st.text_input(k, str(v))) for k, v in db_settings.items()}
...
st.write("Hier können Sie die Einstellungen für Ihre Datenbank und Ihr Modell bearbeiten.")

settings = read_yaml()
edited_settings = display_settings(settings)

if st.button("Speicher Änderungen"):
